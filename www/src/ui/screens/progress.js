...
export function createProgressScreen(el, { core, onBack }) {
  el.innerHTML = `
    <div class="shop-head">
      ...
      <div class="hud-pill">${icon('coin')}<span id="prog-coins">0</span></div>
    </div>
    <div class="shop-tabs">
      ...
      .map((m) => {
        ...
        return `<div class="mission-row${m.done ? ' done' : ''}">
          <div class="mission-top">
            <span class="mission-name">${icon(m.done ? 'check' : 'target')} ${m.name}</span>
            <span class="mission-reward">${m.done ? 'CLAIMED' : `${icon('coin')}${m.reward}`}</span>
          </div>
  ...
  function renderStats() {
    ...
    const rows = [
      [`${icon('target')} Best accuracy`, acc],
      [`${icon('mountain')} Best level`, s.bestLevel || '—'],
      [`${icon('fire')} Best streak`, s.bestStreak],
      [`${icon('sparkles')} Perfects`, s.totalPerfects],
      [`${icon('thumbs')} Goods`, s.goods],
      [`${icon('collision')} Pops`, s.pops],
      [`${icon('worried')} Misses`, s.misses],
      [`${icon('chick')} False starts`, s.falseStarts],
      [`${icon('coin')} Coins earned`, s.coinsEarned],
      [`${icon('swords')} Bosses beaten`, s.bossesBeaten],
      [`${icon('calendar')} Dailies played`, s.dailiesPlayed],
      [`${icon('calendar-alt')} Best daily streak`, s.dailyBestStreak],
      [`${icon('zen')} Zen rounds`, s.zenRounds],
      [`${icon('timer')} Time played`, mins < 60 ? `${mins}m` : `${Math.floor(mins / 60)}h ${mins % 60}m`],
    ];
